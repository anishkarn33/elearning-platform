...
class AuthenticationAPITestCase(BaseAPITestCase):
    ...
    def test_user_can_login(self):
        ...
        data = {"username": self.roger_user.username, "password": "2424df22"}
    ...
    def test_user_can_login_with_email(self):
        ...
        """
        url = reverse("jwt-create")
        data = {"username": self.roger_user.email, "password": "2424df22"}
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, 200)
        expected = ["refresh", "access"]
        for key in expected:
    ...
    def test_user_can_login_with_phone(self):
        ...
        url = reverse("jwt-create")
        data = {"username": self.roger_user.phone, "password": "2424df22"}
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, 200)
        expected = ["refresh", "access"]
        for key in expected:
    ...
    def test_user_can_refresh_token(self):
        ...
        data = {"refresh": response.data.get("data")["refresh"]}
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, 200)
        expected = ["access"]
        for key in expected:
